...
import { getBalance, updateBalance, getRandomTrivia } from '../utils/db.js'
...
export async function checkRobberEvent(message) {
  ...
  LETTERS.forEach((letter, index) => {
    row.addComponents(new ButtonBuilder().setCustomId(`trivia_${userId}_${index}`).setLabel(letter).setStyle(ButtonStyle.Primary))
  })
  ...
  collector.on('end', async (collected, reason) => {
    if (reason === 'time') {
      activeRobberGames.delete(userId)
      const currentBal = await getBalance(userId, username)
      const lostAmount = Math.floor(currentBal * 0.3)
      await updateBalance(userId, username, -lostAmount)
