...
export async function claimDaily(userId, username) {
  ...
  try {
    ...
    if (user.toJSON().lastDaily) {
      ...
      if (vnNow.getUTCFullYear() === vnLast.getUTCFullYear() &&
  ...
  } catch (error) { return { success: false, message: 'Hệ thống đang bảo trì.' }; }
}
...
export async function buyItem(userId, username, itemId, cost) {
  ...
  } catch (e) { return { success: false, message: 'Hỏng đường truyền Cửa hàng!' }; }
}
